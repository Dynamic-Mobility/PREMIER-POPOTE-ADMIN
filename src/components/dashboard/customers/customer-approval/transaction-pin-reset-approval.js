...
const TransactionPinResetApproval = props => {
    ...
    const handleOnApprove = (reject = false) => {
        ...
        setDialogProps({
            open: true,
            message: `Are you sure you want to ${action} transaction pin reset request for ${customer?.name}?`,
            reject: reject,
            showReason: reject,
        })
    };
    ...
    const onApprove = async (reason) => {
        ...
        try{
            const res = await customersApis.approveTxnPinReset(authUser, formData);
            if(res?.success){
...
export default TransactionPinResetApproval;