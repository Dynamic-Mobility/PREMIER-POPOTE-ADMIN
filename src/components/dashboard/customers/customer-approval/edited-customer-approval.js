...
const EditedCustomerApproval = props => {
    ...
    const handleOnApprove = (reject = false) => {
        ...
        setDialogProps({
            open: true,
            message: `Are you sure you want to ${action}  detail changes  for ${customer?.name}?`,
            reject: reject,
            showReason: reject,
        })
    };
    ...
    const onApprove = async (reason) => {
        ...
        try{
            const res = await customersApis.approveUpdatedCustomer(authUser, formData);
            if(res?.success){
...
export default EditedCustomerApproval;