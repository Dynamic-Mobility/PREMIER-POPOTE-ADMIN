...
const ResetSecurityQuestionsApproval = props => {
    ...
    const handleOnApprove = (reject = false) => {
        ...
        setDialogProps({
            open: true,
            message: `Are you sure you want to ${action} security questions reset request for ${customer?.name}?`,
            reject: reject,
            showReason: reject,
        })
    };
    ...
    const onApprove = async (reason) => {
        ...
        try{
            const res = await customersApis.approveSecurityQuestionsReset(authUser, formData);
            if(res?.success){
...
export default ResetSecurityQuestionsApproval;