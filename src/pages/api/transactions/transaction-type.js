...
export default async function handler(req, res) {
    if (req.method === API_METHODS.GET) {
        try {
            ...
            await backendAxiosInstance.get(`${API_URL.GET_TRANSACTION_TYPE}`, config).then(response => {
        ...
        try {
            ...
            const body = req.body;
            console.log("FILTER TRANSACTIONS ",body);
            await backendAxiosInstance.post(`${API_URL.FILTER_TRANSACTIONS}/${body}`, body, config)
                .then(response => {
                    res.status(200).json(response.data);
                })
                .catch(e => {
