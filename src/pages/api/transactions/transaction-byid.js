...
export default async function handler(req, res) {
    if (req.method === API_METHODS.GET) {
        try {
            ...
            await backendAxiosInstance.get(`${API_URL.GET_TRANSACTIONBY_ID}/${trnId}`, config).then(response => {
        ...
        try {
            ...
            const body = req.body;
            await backendAxiosInstance.post(`${API_URL.UPLOAD_FILE}/${body.sponsorId}/${body.branchId}`, body, config)
                .then(response => {
                    res.status(200).json(response.data);
                })
                .catch(e => {
