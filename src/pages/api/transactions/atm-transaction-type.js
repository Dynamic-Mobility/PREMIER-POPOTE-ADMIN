...
export default async function handler(req, res) {
    if (req.method === API_METHODS.GET) {
        try {
            ...
            const body = req.body
            await backendAxiosInstance.get(`${API_URL.GET_ATM_TRANSACTION_TYPE}`, config).then(response => {
        ...
        try {
            ...
            const body = req.body;
            console.log("URL ",`${API_URL.GET_ATM_TRANSACTION_BY_TYPE}/${body.transactionId}`)
            await backendAxiosInstance.get(`${API_URL.GET_ATM_TRANSACTION_BY_TYPE}/${body.transactionId}`, {}, config)
                .then(response => {
                    res.status(200).json(response.data);
                })
                .catch(e => {
