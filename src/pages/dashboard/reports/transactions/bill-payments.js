...
import {
    ...
    resetFilters,
    setFilters
} from "../../../../slices/dashboard/transactions/bill-transactions";
...
const BillPaymentPage = () => {
    ...
    const {
        ...
        activePage,
        totalRecords
    } = useSelector(({ billTransactions }) => billTransactions);

    const handleOnPageSizeChange = async value => {
        await getBillTransactions(filters, value, 1);
        dispatch(setPageSize(value));
        dispatch(setActivePage(1));
    }
    const handleOnPageChange = async value => {
        await getBillTransactions(filters, pageSize, value);
...
BillPaymentPage.getLayout = (page) => (
    <AuthGuard>
        <ModernLayout>
            {page}
        </ModernLayout>
